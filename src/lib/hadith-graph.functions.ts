...
/**
 * Embed a batch of hadith entries (english_text fallback arabic_text) that
...
export const embedHadithBatch = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const inputs = rows.map((r) =>
      clip(
        [r.narrator ?? "", r.english_text ?? r.arabic_text ?? ""]
          .filter(Boolean)
          .join(" — "),
      ),
    );
...
/**
 * For each embedded hadith without graph links yet, find the most similar
...
export const linkHadithToGraph = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    // Skip hadith already linked at all.
