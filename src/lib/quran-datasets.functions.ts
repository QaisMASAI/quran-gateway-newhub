  ...
  .handler(async ({ context, data }) => {
    ...
    for (let i = 0; i < records.length; i += BATCH) {
      ...
      const { error } = await supabaseAdmin
        ...
        .upsert(slice, { onConflict: "dataset_id,external_key" });
      if (error) throw new Error(error.message);
...
export const upsertQuranWordAnnotations = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const { error } = await supabaseAdmin
      ...
      .upsert(data.rows, { onConflict: "surah,ayah,word_index" });
    if (error) throw new Error(error.message);
  ...
  .handler(async ({ context, data }) => {
      ...
      .upsert(
        {
          ...
          metadata: data.reciter.metadata,
          is_active: true,
        },
        { onConflict: "code" },
      )
    ...
    for (let i = 0; i < rows.length; i += BATCH) {
      ...
      const { error } = await supabaseAdmin
        ...
        .upsert(slice, { onConflict: "reciter_id,surah,ayah,quality_label" });
      if (error) throw new Error(error.message);
    }
