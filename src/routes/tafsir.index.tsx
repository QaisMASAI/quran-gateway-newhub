...
export const Route = createFileRoute("/tafsir/")({
  head: () => {
    ...
    const title =
      locale === "ar"
        ? "أعظم منصة تفسير في العالم | بيان القرآن والتفسير"
        : locale === "he"
          ? "פלטפורמת תפסיר המתקדמת בעולם | הסבר האסלאם"
          : "World's Most Advanced Tafsir Platform | Bayan Al-Quran and Tafsir";
    const description =
      locale === "ar"
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
            ...
            >
              <div className="flex items-center gap-2 font-bold text-sm text-foreground border-b border-border/60 pb-3">
                ...
                <span>
                  {locale === "ar" ? "الانتقال السريع لآية" : locale === "he" ? "מעבר מהיר לפסוק" : "Jump to Any Verse"}
                </span>
              </div>
        ...
        <section className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {TAFSIR_SOURCES_META.map((meta) => {
              ...
              const author = locale === "ar" ? meta.author_ar : locale === "en" ? meta.author_en : meta.author_he;
              const description =
                locale === "ar" ? meta.description_ar : locale === "en" ? meta.description_en : meta.description_he;
                ...
                >
                  <div className="space-y-3">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <span className="text-[10px] font-mono text-muted-foreground">{meta.era.split("/")[0]}</span>
                    </div>
                    ...
                    <p className="text-xs text-foreground/80 leading-relaxed line-clamp-3">{description}</p>
                  </div>
        ...
        <section className="rounded-3xl border border-border/80 bg-card p-6 md:p-8 shadow-sm space-y-6">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-bold text-foreground flex items-center gap-2">
              ...
              <span>{locale === "ar" ? "تفاسير مختارة لآيات عظيمة" : "Featured Key Verses Commentary"}</span>
            </h2>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
            {featuredVerses.map((v) => (
              ...
              >
                <span className="font-mono text-xs font-bold text-primary mb-1">Verse {v.verse}</span>
