  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        if (entErr)
          return Response.json({ step: "entities", error: entErr.message }, { status: 500 });
        ...
        if (fetchErr)
          return Response.json({ step: "fetch", error: fetchErr.message }, { status: 500 });
        ...
        if (relRows.length > 0) {
          ...
          if (rErr)
            return Response.json({ step: "relations", error: rErr.message }, { status: 500 });
        }
