...
function AdminKids() {
  ...
  const save = async () => {
    ...
    const { error } = await supabase.from("kids_questions").insert({
      ...
      related_ref: draft.related_ref || null,
      ...
      const payload = lines
        ...
        .map((cells) => {
          ...
          return {
            ...
            options,
            answer_index: Number.isFinite(answerRaw) ? Math.max(0, Math.min(3, answerRaw)) : 0,
        ...
        .filter(
          (row) =>
            row.question_kind === "interactive" ||
            row.options.every((option) => option.trim().length > 0),
        );
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8">
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        Manage quiz questions shown in the Kids Zone.
      </p>
      {msg && (
        <p className="mt-3 rounded-lg border border-border bg-card px-3 py-2 text-sm">{msg}</p>
      )}
      ...
      <section className="mt-6 rounded-2xl border border-border bg-card p-4">
        <div className="mb-3 flex flex-wrap items-center gap-2">
          ...
          <Select
            value={filterAge}
            onValueChange={(value) => setFilterAge(value as "all" | "kids" | "young")}
          >
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="text-xs text-muted-foreground">
            Showing {filteredRows.length} of {rows.length} questions
          </span>
        </div>
      </section>
      ...
      <section className="mt-6 rounded-2xl border border-border bg-card p-5">
        <h2 className="mb-4 flex items-center gap-2 text-lg font-bold">
          <Plus className="h-5 w-5 text-primary" /> New question
        </h2>
    ...
    <div className="grid gap-3 sm:grid-cols-2">
      ...
      <label className="text-sm">
        ...
        <select
          ...
          onChange={(e) => set("age_group", e.target.value as "kids" | "young")}
      ...
      <label className="text-sm">
        ...
        <Input
          ...
          onChange={(e) => set("difficulty", Math.max(1, Math.min(3, Number(e.target.value))))}
          className="mt-1"
        />
      </label>
      <label className="text-sm">
      ...
      {value.options.map((opt, i) => (
        <label key={i} className="text-sm">
          Option {i + 1}{" "}
          {value.answer_index === i && <span className="text-emerald-600">(correct)</span>}
          <div className="mt-1 flex gap-2">
            <Input
              ...
              onChange={(e) =>
                set(
                  "options",
                  value.options.map((o, j) => (j === i ? e.target.value : o)),
                )
              }
              className="flex-1"
            />
      ...
      <label className="text-sm sm:col-span-2">
        Related reference (surah:ayah or topic slug)
        <Input
          value={value.related_ref ?? ""}
