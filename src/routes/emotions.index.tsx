...
function EmotionCard({
  ...
  return (
    ...
    >
      <div
        className={`pointer-events-none absolute inset-0 bg-gradient-to-bl ${ACCENT[accent]} opacity-60`}
        aria-hidden
      />
      <div className="relative flex items-start justify-between gap-3">
        ...
        <ChevronLeft
          className="mt-2 h-4 w-4 shrink-0 text-muted-foreground transition-all group-hover:-translate-x-0.5 group-hover:text-gold rtl:rotate-0 ltr:rotate-180"
          aria-hidden="true"
        />
      </div>
      <div className="relative min-w-0">
        ...
        <p className="mt-3 text-[11px] font-medium text-gold">
          {refsCount} {t("emotions.selectedVerses")}
        </p>
      </div>
    </Link>
  );
