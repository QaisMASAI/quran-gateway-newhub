        ...
        {deferredTrimmed.length >= 2 && (
          <section className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
            <ResultPill
              ...
              label="Verses"
              value={groupedResultCounts.verses}
            />
            <ResultPill
              icon={<Library className="h-3.5 w-3.5" />}
              label="Tafsir"
              value={groupedResultCounts.tafsir}
            />
            <ResultPill
              icon={<Brain className="h-3.5 w-3.5" />}
              label="Hadith"
              value={groupedResultCounts.hadith}
            />
            <ResultPill
            ...
            <div className="space-y-2">
              {textsQ.data.map((row) => (
                <article key={`${row.kind}-${row.id}`} className="surface-card px-4 py-3">
                  ...
                  {(row.kind === "tafsir" || row.kind === "asbab") &&
                  row.surah &&
                  row.ayah_start ? (
                    <Link
                      to="/surah/$id"
            ...
            <div className="space-y-3">
              {hadithItems.map((h) => (
                ...
                >
                  ...
                  {h.narrator ? (
                    <p className="text-[11px] italic text-muted-foreground">{h.narrator}</p>
                  ) : null}
                  {h.english_text ? (
