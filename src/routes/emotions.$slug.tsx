  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="mx-auto max-w-xl px-6 py-20 text-center">
        <h1 className="mb-2 font-display text-2xl font-bold text-primary">{t("detail.notFoundEmotion")}</h1>
        <Link to="/emotions" className="text-sm text-gold underline">{t("detail.backToEmotions")}</Link>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main id="main" className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
        <Link to="/emotions" className="inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-primary">
        ...
        <header className="mt-6 rounded-3xl border border-primary/10 bg-card p-8 text-center shadow-sm">
          ...
          <p className="mx-auto mt-4 max-w-xl text-sm leading-relaxed text-foreground/80">{e.description}</p>
        </header>

        <div className="mt-6"><TrustBadge variant="block" /></div>
